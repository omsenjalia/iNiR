...
Power profile management using UPower. Cycle between profiles or set a specific profile directly.

| Function | Description |
|----------|-------------|
| `cycle` | Cycle to the next power profile |
| `set <name>` | Set a specific power profile (e.g. `power-saver`, `balanced`, `performance`) |

```kdl
bind "XF86Launch4" { spawn "inir" "powerProfile" "cycle"; }
```
